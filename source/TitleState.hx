...
class TitleState extends MusicBeatState
{
	...
	var easterEggKeyCombination:Array<FlxKey> = [FlxKey.T, FlxKey.W, FlxKey.I, FlxKey.S, FlxKey.H,];
	...
	function startIntro()
	{
		...
		backgroundtitle = new FlxSprite().loadGraphic(Paths.image('spaceBG'));
		...
		FlxTween.tween(logo, {x: logoBl.x + 100}, 0.6, {ease: FlxEase.quadInOut, type: PINGPONG, startDelay: 0.1});
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				case 1:
					#if PSYCH_WATERMARKS
					createCoolText(['Twish Engine by'], 15);
					#else
				...
				case 3:
					#if PSYCH_WATERMARKS
					addMoreText('Twish', 15);
					#else
					addMoreText('Present You To');
					#end
				// credTextShit.text += '\npresent...';
